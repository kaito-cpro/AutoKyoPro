...
def submit(args: 'argparse.Namespace') -> None:
    ...
    with utils.with_cookiejar(utils.new_default_session(), path=args.cookie) as sess:
        ...
        if args.language in langs:
            ...
            if args.guess:
                kwargs = {
                    ...
                    'python_version': '3', # 自分で書き換えた箇所(元は '3' ではなく args.guess_python_version)
        ...
        if args.open:
            # 自分で書き換えた箇所
            subprocess.check_call([ 'explorer', submission.get_url() ], stdin=sys.stdin, stdout=sys.stdout, stderr=sys.stderr)
    ...
    if ext in ( 'cpp', 'cxx', 'cc', 'C' ):
        ...
        if select_words([ 'python', '2' ], lang_ids) and select_words([ 'python', '3' ], lang_ids):
            ...
            for version in versions:
                ...
                lang_ids += select('python %d' % version, saved_ids)
                # 自分で書き換えた箇所(AtCoder の judge で言語がアップデートされたら追加する)
                lang_ids += select('python(%d' % version, saved_ids)
                lang_ids += select('python (%d' % version, saved_ids)
